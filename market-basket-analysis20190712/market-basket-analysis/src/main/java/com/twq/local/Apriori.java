...
public class Apriori {
    ...
    /**
     *  计算所有的pass+1-项候选集(即我们说的k+1-项候选集)
        ...
        for (int i = 0; i < frequentItemSets.size(); i++) {
            ...
            for (int j = i + 1; j < frequentItemSets.size(); j++) {
                ...
                if (isCandidate) {
                    ...
                    union.addAll(s2); //因为这里是Set，所以会消除重复的
    ...
    private boolean isCandidate(int pass, Set<String> s1, Set<String> s2) {
        ...
        intersection.retainAll(s2); // 取项集s1和s2的交集
